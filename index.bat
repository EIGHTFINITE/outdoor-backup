...
:git
for /f "tokens=1,2 delims=:, " %%a in (' find ":" ^< "pagelist.json" ') do (
  set "%%~a-version=%%~b"
)
if not exist "bin\windows\x64\git\PortableGit-%portable-git-short-version%-64-bit\bin\git.exe" (
  echo Missing git. Downloading it now...
  curl.exe -#Lo"artifacts.zip" https://github.com/EIGHTFINITE/outdoor-backup/archive/refs/tags/artifacts.zip
...
call:git remote add origin https://github.com/EIGHTFINITE/outdoor-backup.call:git 2>nul
call:git remote set-url origin https://github.com/EIGHTFINITE/outdoor-backup.git
