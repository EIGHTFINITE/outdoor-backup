...
# 7z Windows x64
p7zip_version=$(cat pagelist.json | python -c "import sys, json; print(json.load(sys.stdin)['config']['p7zip-version'])")
p7zip_short=$(cat pagelist.json | python -c "import sys, json; print(json.load(sys.stdin)['config']['p7zip-short'])")
curl -sSLo "7z$p7zip_short-x64.exe" --header "Authorization: token $GITHUB_TOKEN" https://github.com/ip7z/7zip/releases/download/$p7zip_version/7z$p7zip_short-x64.exe
mkdir -p "bin/windows/x64/7z/7z$p7zip_short-x64"
7z x -o"bin/windows/x64/7z/7z$p7zip_short-x64" "7z$p7zip_short-x64.exe" | grep "ing archive"
rm "7z$p7zip_short-x64.exe"
rm "bin/windows/x64/7z/7z$p7zip_short-x64/Uninstall.exe"
sed -i '/\/bin\//d' -- '.gitignore'
git add "bin/windows/x64/7z/7z$p7zip_short-x64"
...
portable_git_short=$(cat pagelist.json | python -c "import sys, json; print(json.load(sys.stdin)['config']['portable-git-short'])")
portable_git_version=$(cat pagelist.json | python -c "import sys, json; print(json.load(sys.stdin)['config']['portable-git-version'])")
curl -sSLo "PortableGit-$portable_git_short-64-bit.7z.exe" --header "Authorization: token $GITHUB_TOKEN" https://github.com/git-for-windows/git/releases/download/v$portable_git_version/PortableGit-$portable_git_short-64-bit.7z.exe
mkdir -p "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit"
7z x -o"bin/windows/x64/git/PortableGit-$portable_git_short-64-bit" "PortableGit-$portable_git_short-64-bit.7z.exe" | grep "ing archive"
rm "PortableGit-$portable_git_short-64-bit.7z.exe"
rm "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/post-install.bat"
rmdir "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/dev/mqueue"
rmdir "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/dev/shm"
rmdir "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/tmp"
git submodule add -f -- https://github.com/EIGHTFINITE/void.git "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/dev/mqueue"
git submodule add -f -- https://github.com/EIGHTFINITE/void.git "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/dev/shm"
git submodule add -f -- https://github.com/EIGHTFINITE/void.git "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit/tmp"
sed -i '/\/bin\//d' -- '.gitignore'
git add .gitmodules "bin/windows/x64/git/PortableGit-$portable_git_short-64-bit"
git -c user.name="GitHub" -c user.email="noreply@github.com" commit --author="github-actions[bot] <41898282+github-actions[bot]@users.noreply.github.com>" -m"Add Windows x64 PortableGit $portable_git_short release artifacts" | sed -n 1p
git checkout -- '.gitignore'
if [[ $(git status --porcelain | tee /dev/stderr | head -c1 | wc -c) -ne 0 || $(git clean -dffx | tee /dev/stderr | head -c1 | wc -c) -ne 0 ]]
  then exit 1
fi
...
# uBlock Origin
ublock_origin_version=$(cat pagelist.json | python -c "import sys, json; print(json.load(sys.stdin)['config']['ublock-origin-version'])")
