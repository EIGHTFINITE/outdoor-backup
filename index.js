	...
	app.whenReady().then(async () => {
		...
		const pageLists = async () => {
			...
			for (let i = 0; i < pages.length; i++) {
				...
				await browserWindow.webContents.executeJavaScript(`[[...document.querySelectorAll('a[href]')].map(a => a.href), window.performance.getEntries().find(e => e.entryType === 'navigation').responseStatus, (document.querySelectorAll('body>*').length === 1 && document.querySelectorAll('body>img').length), document.querySelectorAll('#webkit-xml-viewer-source-xml').length]`).then((results) => {
					...
					for(let j = 0; j < results.length; j++) {
						...
						if(!results[j].startsWith('https://')) {
							throw Error('Unable to map ' + results[j])
						}
						results[j] = new URL(results[j]);
						results[j].hash = '';
						results[j] = results[j].toString();
						if(results[j].startsWith('https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/image/cache/full/') || results[j].startsWith('https://asset.myonlinestore.eu/winkel/outdoorsportshop/images/full/')) {
							// Image
							results[j] = new URL(results[j]);
							results[j].search = '';
							results[j] = results[j].toString();
