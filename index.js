...
if(process.versions.electron) {
	...
	app.whenReady().then(async () => {
		...
		const pageLists = async () => {
			...
			for (let i = 0; i < pages.length; i++) {
				console.log('Navigating to ' + pages[i][0])
				await browserWindow.loadURL(pages[i][0])
				await browserWindow.webContents.executeJavaScript(`[
					[...[...document.querySelectorAll('a[href]')].map(a => a.href), ...[...document.querySelectorAll('img[src]')].map(a => a.src)],
					...
					if(isImage) {
						// Image
						pagelist.pages[pages[i][0]].type = 'img'
						return
					}
					if(isXmlViewer) {
						// XML
						pagelist.pages[pages[i][0]].type = 'xml'
						return
					}
					if(isProduct) {
					...
					for(let j = 0; j < results.length; j++) {
						if(results[j].startsWith('javascript:')) {
							// JavaScript
							continue
						}
						if(results[j].startsWith('mailto:')) {
							// Email
							continue
						}
						if(results[j].startsWith('http://')) {
							// Upgrade http to https
							results[j] = 'https://' + results[j].slice(7)
						}
						if(!results[j].startsWith('https://')) {
							throw Error('Unable to parse ' + results[j])
						}
						results[j] = new URL(results[j]);
						results[j].hash = '';
						results[j] = results[j].toString();
						if(results[j].startsWith('https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/') || results[j].startsWith('https://asset.myonlinestore.eu/winkel/outdoorsportshop/images/full/')) {
							// Image
							results[j] = new URL(results[j]);
							results[j].search = '';
							results[j] = results[j].toString();
							if(results[j] === 'https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/logo_large.png'
							|| results[j] === 'https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/images/thuisvergelijken-footer.jpg'
							|| results[j] === 'https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/images/PostNL-footer.jpg'
							|| results[j] === 'https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/images/1200px-Bancontact_logo.svg.jpg'
							|| results[j] === 'https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/images/wero.png'
							|| results[j] === 'https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/images/logo2.jpg') {
								// Global Image
							...
							if(results[j].startsWith('https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/image/cache/full/')) {
								// Avoid duplicate images
								results[j] = 'https://asset.myonlinestore.eu/winkel/outdoorsportshop/images/full/' + results[j].slice(83)
							}
							if(results[j].startsWith('https://cdn.myonlinestore.eu/94881a4c-6be1-11e9-a722-44a8421b9960/image/cache/article/')) {
								// Thumbnails to full size images
								results[j] = 'https://asset.myonlinestore.eu/winkel/outdoorsportshop/images/full/' + results[j].slice(86)
							}
						...
						if(!results[j].startsWith('https://www.outdoorsportshop.nl/')) {
							// External
							if(results[j].startsWith('https://www.youtube-nocookie.com/embed/')) {
								// YouTube
								results[j] = new URL(results[j]);
								results[j].search = '';
								results[j] = results[j].toString();
								results[j] = 'https://www.youtube.com/watch?v=' + results[j].slice(39)
								pagelist.pages[pages[i][0]].youtube = results[j];
								continue
							}
							if(results[j].startsWith('https://img.youtube.com/vi/')) {
								// YouTube Thumbnail
								continue
							}
							if(!pagelist.external.includes(results[j])) {
								pagelist.external.push(results[j])
							}
							continue
						}
						if(results[j].startsWith('https://www.outdoorsportshop.nl/_gcb/generate-captcha/')) {
							// Captcha
							continue
						}
						// Webpage
			...
			fs.writeFileSync('pagelist.json', stringify(pagelist), 'utf-8')
			browserWindow.close()
		}
		pageLists()
	...
	// Unpack Electron
	if(isWindows) {
		const deleteFilesStartsWith = require('./lib/deleteFilesStartsWith.js')
