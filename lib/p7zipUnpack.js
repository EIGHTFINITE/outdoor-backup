...
const p7zipUnpack = (file, out, type, callback) => {
	const { status } = spawnSync('bin\\windows\\x64\\7z\\7z' + require('../pagelist.json').config['p7zip-version'] + '-x64\\7z.exe', (type === null ? ['x', '-ba', '-aoa', '-o' + out, file] : ['x', '-ba', '-aoa', '-o' + out, type, file]), { stdio: 'inherit' })
	if(status !== 0) {
